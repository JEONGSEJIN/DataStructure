...
def Topo_sort(head, in_degree) :
    stack = []# 진입 차수가 0인 정점을 넣어줄 스택을 하나 생성
    for i in in_degree :#진입 차수 중
        if i.link.vertex == 0 : # 차수가 0인게 있다면
            stack.append(i.vertex) # 스택에 삽입
    print("\n\n위상 정렬 순서 :") 
    while stack != [] :# 빈 스택이 될 때까지 반복
        v = stack.pop() # 스택에서 요소를 하나 pop
        print(v, "->",end = ' ') # 출력
        vert = [] # 인접 정점을 담기 위한 빈 리스트
        for i in head :#정점 리스트 중에서
            if i.vertex == v :#정점리스트의 요소와 스택의 요소가 같다면
                while i.link.vertex != None : # 정점 리스트에서 인접 정점들을 모두 
                    vert.append(i.link.vertex) # vert리스트에 저장해 준다.
                    i.link = i.link.link
                break
        for j in in_degree :#진입 차수 중
            for a in vert :#vert 리스트 중
                if j.vertex == a : # 진입 차수와 vert리스의 요소가 같은 게 있다면
                    j.link.vertex -= 1 # 진입 차수를 하나씩 빼줌
                    if j.link.vertex == 0 : # 만약 빼준 진입 차수중 0이 있다면 
                        stack.insert(0,j.vertex) #스택에 진입 차수가 0인 정점을 삽입
                        j.link = j.link.link
                        break
...
def main() :
    ...
    ####################################################
    rand_list = random_int(len(in_degree))
    temp_list = copy.deepcopy(in_degree)
    index = 0

    for i in in_degree:
        temp_list[rand_list[index]] = i   
        index += 1
